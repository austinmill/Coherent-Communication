...
#####################################################################
'''
def add_phase_noise1(Sin, Fs, phase_noise_freq, phase_noise_power, VALIDATION_ON=False):
    """
    ...
    for i, phase_noise_freq_i in enumerate(phase_noise_freq):
        for j, phase_noise_freq_sorted_j in enumerate(phase_noise_freq_sorted):
            if j == 0 and phase_noise_freq_i < phase_noise_freq_sorted_j:
                phase_noise_freq_sorted  = np.insert(phase_noise_freq_sorted, 0, phase_noise_freq_i)
                phase_noise_power_sorted = np.insert(phase_noise_power_sorted, 0, phase_noise_power[i])
                break
            elif i != 0 and \
                    j == phase_noise_freq_sorted.shape[0] - 1 and \
                    phase_noise_freq_i >= phase_noise_freq_sorted_j:
                phase_noise_freq_sorted  = np.append(phase_noise_freq_sorted, phase_noise_freq_i)
                phase_noise_power_sorted = np.append(phase_noise_power_sorted, phase_noise_power[i])
                break
            else:
                if i != 0 and \
                        j != phase_noise_freq_sorted.shape[0] - 1 and \
                    ...
                    phase_noise_freq_sorted  = np.insert(phase_noise_freq_sorted, j + 1, phase_noise_freq_i)
                    phase_noise_power_sorted = np.insert(phase_noise_power_sorted, j + 1, phase_noise_power[i])
                    break
    ...
    if (1):#VALIDATION_ON:
        plt.figure()
        #———————————————————————#Phase Noise
        fpY = fft(phase_noise)[: M]
        fpY = abs(fpY)**2
        fpY = fpY/np.max(fpY)
        fpY = 10*np.log10(fpY/dF[0])
        plt.semilogx(F, fpY, label='Positive spectrum of the generated phase noise exp(j * x)')
        #X1 = fft(phase_noise)
        #plt.plot(F, 10 * np.log10(((np.abs(X1[: M])/np.max(np.abs(X1[: M])))**2)/dF[0]), color='k', \
        #             label='Positive spectrum of the generated phase noise exp(j * x)')
        #———————————————————————#
        X2 = fft(1 + 1j * np.real(x[: N]))
        #plt.plot(F, 10 * np.log10(((np.abs(X2[: M])/np.max(np.abs(X2[: M])))**2)/dF[0]), 'm', \
        #            label='Positive spectrum of the approximation (1 + j * x)')
        #———————————————————————#Input Mask
        plt.plot(F, 10 *np.log10(P), color='red', label='Interpolated SSB phase noise mask')
        plt.scatter(phase_noise_freq, phase_noise_power, label='Input SSB phase noise mask', color='red', s=30)
        #———————————————————————#
        plt.grid()
        plt.xlabel('Frequency [Hz]')
        plt.ylabel('dBc/Hz')
        plt.xlim(phase_noise_freq[1],Fs/2)
        plt.ylim(-163,3)
        plt.legend()
        plt.show()
...
# %%
def test(length=1e4,VALIDATION_ON=False):
    Fc = 3e3    # carrier frequency
    Fs = 40e6    # sampling frequency
    t = np.arange(0, length)
    S = np.exp(1j * 2 * np.pi * Fc/Fs * t)  # complex sinusoid
    ...
    phase_noise_freq = np.array([1e3, 10e3, 100e3, 1e6, 10e6])  # Offset From Carrier
    phase_noise_power = np.array([-84, -96, -100, -109, -122])  # Phase Noise power
    
    #phase_noise_freq = np.array([1e3, 10e3, 100e3, 1e6, 10e6])  # Offset From Carrier
    #phase_noise_power = np.array([-24, -46 , -60  , -89, -100])  # Phase Noise power
    S1 = add_phase_noise1(S, Fs, phase_noise_freq, phase_noise_power, VALIDATION_ON)
...
————————————————
版权声明：本文为CSDN博主「行客Thinker」的原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/sinat_28113749/article/details/78598399
'''
